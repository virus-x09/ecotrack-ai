...
@app.post("/image-compare")
async def image_compare(
    photo1: UploadFile = File(...),
    photo2: UploadFile = File(...),
    authorization: Optional[str] = Header(None),
) -> dict[str, object]:
    """Compare two images and return TRUE only for a 75%+ visual match."""
    authenticated_user(authorization)
    allowed_types = {"image/jpeg", "image/png", "image/webp"}
    if photo1.content_type not in allowed_types or photo2.content_type not in allowed_types:
        raise HTTPException(status_code=400, detail="Only JPEG, PNG, and WebP images are supported")
    first_bytes, second_bytes = await photo1.read(), await photo2.read()
    if not first_bytes or not second_bytes:
        raise HTTPException(status_code=400, detail="Both images are required")
    if len(first_bytes) > 10 * 1024 * 1024 or len(second_bytes) > 10 * 1024 * 1024:
        raise HTTPException(status_code=413, detail="Each image must be smaller than 10 MB")
    try:
        return compare_image_bytes(first_bytes, second_bytes)
...
def request_otp(request: RequestOtpRequest) -> dict[str, str]:
    email = request.email.strip().lower()
    stored_user = find_user_by_email(email)
    if not stored_user:
